...
test("server-renders the complete PAGER investor story", async () => {
  ...
  assert.match(visibleText, /Один аккаунт\. Один постоянный\s+PAGER\s+ID\./);
  assert.match(visibleText, /Каждый контакт получает собственные правила общения/);
  assert.match(visibleText, /PAGER\s+создает новую модель цифрового общения/);
  assert.match(html, /data-scene="16"/);
  assert.match(html, /PAGER ID/);
  assert.match(visibleText, /Контакт не становится доступом автоматически/);
  assert.match(html, /class="relationship-console"/);
  assert.match(html, /class="act-progress"/);
  assert.match(html, /class="access-shift"/);
  assert.match(html, /class="relationship-thread/);
